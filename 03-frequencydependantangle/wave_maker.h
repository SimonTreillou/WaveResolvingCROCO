...
!
        ...
        !MPI_master_only write(stdout,*) wd
        wds=wmaker_dsp    ! directional spread
        !MPI_master_only write(stdout,*) wds
        gamma=wmaker_fsp  ! JONSWAP peakedness parameter
        !MPI_master_only write(stdout,*) gamma
#endif
!
...
!
        coswd=cos(wd)
        !MPI_master_only write(stdout,*) coswd
        sinwd=sin(wd)
#ifdef WAVE_MAKER_EAST
        coswd=cos(wd+180.0*deg2rad)
        sinwd=sin(wd+180.0*deg2rad)
#endif
        coswds=cos(wds)
        sinwds=sin(wds)
!
...
!
        h0=h(IB0,1)
        !MPI_master_only write(stdout,*) h0
...
!
          ...
          cff0=0.
          do jw=1,Nfrq
            !write(stdout,*) wd_bry(jw)*180.0/pi
            !write(stdout,*) wa_bry_d(jw)
            !write(stdout,*) wa_bry_f(jw)
            !write(stdout,*) ' '
...
#  endif /* WAVE_MAKER_DSPREAD_PER  */
          ...
          do iw=1,Nfrq
#   ifdef WAVE_MAKER_EAST
             wd_bry(jw)=wd_bry(jw)+180.0*deg2rad
#   endif
